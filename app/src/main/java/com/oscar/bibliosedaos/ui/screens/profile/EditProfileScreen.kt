...
fun EditProfileScreen(
    ...
    /**
     * ID de l'usuari que s'està editant.
     * Si userId és null, s'edita el propi perfil.
     */
    val editingUserId = userId ?: loginState.authResponse?.id
    ...
    /**
     * Carrega el perfil de l'usuari que s'està editant
     */
    LaunchedEffect(editingUserId) {
        editingUserId?.let { id ->
            authViewModel.loadUserProfile(id)
        }
    }
    ...
    LaunchedEffect(updateUserState.isUpdating, updateUserState.error) {
        if (hasSubmitted && !updateUserState.isUpdating) {
            if (updateUserState.error == null && updateUserState.success) {
                ...
                hasSubmitted = false
                // Tornar al perfil de l'usuari que s'ha editat
                val targetUserId = editingUserId ?: loginState.authResponse?.id ?: 0L
                navController.navigate(
                    AppScreens.UserProfileScreen.createRoute(targetUserId)
                ) {
                ...
                navigationIcon = {
                    if (!isNavigating) {
                        IconButton(onClick = {
                            if (!isNavigating) {
                                isNavigating = true
                                // Tornar al perfil de l'usuari que s'estava editant
                                val targetUserId = editingUserId ?: loginState.authResponse?.id ?: 0L
                                navController.navigate(
                                    AppScreens.UserProfileScreen.createRoute(targetUserId)
                                ) {
                ...
                userProfileState.user != null -> {
                    ...
                    Button(
                        onClick = {
                            hasSubmitted = true
                            editingUserId?.let { id ->
                                // Si s'està editant un altre usuari, usar updateUserProfile
                                // Si s'està editant el propi perfil, usar updateCurrentUserProfile
                                if (id == loginState.authResponse?.id) {
